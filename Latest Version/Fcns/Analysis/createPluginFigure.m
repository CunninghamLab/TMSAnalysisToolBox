...
%}
function [CustomAnalysisOpts,UserVar]=createPluginFigure(app,existFig,appCustomAnalysisOpts,PluginsFolderName,numVar,ListofVariableLabels,PluginName,DefaultValues)
...
if existFig == 0 %pop-up doesn't exist yet - build it
    ...
    %Reload this plugin's code (clear + rehash) so edits to the plugin file
    %are picked up on the next run, without using the command window.
    ReloadButton=uibutton(Grid,'text','Reload code','ButtonPushedfcn', @(src,event) deferReload(PluginName));
    ReloadButton.Layout.Row=numRows+1;
    ReloadButton.Layout.Column=2;
...
% =================== reload-button helpers (do not edit) ===============
function deferReload(pluginName)
% Reload the plugin's code as soon as it stops running. The plugin (and this
% helper) are on the call stack while the pop-up is open, so clearing now is a
% no-op; a timer retries until the clear succeeds, then rehashes and stops.
tmr = timer('ExecutionMode','fixedSpacing', 'StartDelay',0.3, 'Period',0.5, ...
            'TasksToExecute',120, 'UserData',pluginName, ...
            'TimerFcn',@reloadTick, 'StopFcn',@(t,~) delete(t));
start(tmr);
end

function reloadTick(tt,~)
name = tt.UserData;
clear(name);                  % no-op while the plugin is still running
if ~ismember(name, inmem)     % cleared -> the plugin has returned
    rehash;
    stop(tt);
end
end
